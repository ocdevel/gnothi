...
from catboost import Pool, CatBoostRegressor, cv as cb_cv
...
class XGBHyperOpt:
    ...
    def objective(self, trial):
        ...
        if use_catboost:
            pool = Pool(self.x.fillna(0), self.y)
            scores = cb_cv(pool=pool,
               params={"loss_function": "MAE", **param}, # "task_type": "GPU"
               num_boost_round=args["num_boost_round"],
               seed=SEED,
               verbose=False,
               early_stopping_rounds=args["early_stopping_rounds"])
            best_score = scores['test-MAE-mean'].values[-1]
        else:
        ...
        # Using CatBoost because above hypers can have categorical values, which XGBoost doesn't support.
        # Also, CatBoost evidently good without hyper-optimizing. Thus, consider replacing XGBoost above with CatBoost?
