...
class Shares:
    ...
    async def _shares_put_post(d, data, share_id=None):
        user, db, send = d.user, d.db, d.mgr.send_other
        data = data.dict()
        tags = data.pop('tags')
        data['fields'] = data['fields_'];
        del data['fields_']  # pydantic conflict
        if share_id:
            s = db.query(M.Share).filter_by(user_id=user.id, id=share_id).first()
        ...
        for tag, v in tags.items():
            if not v: continue
            db.add(M.ShareTag(share_id=s.id, tag_id=tag))
        db.commit()
    ...
    async def on_delete(data: BM_ID, d):
        if d.snooping: raise CantSnoop()
        d.db.query(M.Share).filter_by(user_id=d.user.id, id=data.id).delete()
        d.db.commit()
