...
class Tags:
    ...
    async def on_tags_get(data: BM, d) -> List[PyT.TagOut]:
        return M.Tag.snoop(d.db, d.viewer.email, d.user.id, snooping=d.snooping).all()
    ...
    async def on_tag_put(data: PyT.TagIn, d):
        if d.snooping: raise CantSnoop()
        tag = d.db.query(M.Tag).filter_by(user_id=d.user.id, id=data.id).first()
        data = data.dict()
    ...
    async def on_tag_delete(data: BM_ID, d):
        if d.snooping: raise CantSnoop()
        tagq = d.db.query(M.Tag).filter_by(user_id=d.user.id, id=data.id)
        if tagq.first().main:
