...
const TS_FORMAT = `\`\`\`typescript
{
  ...
  // An array of core concepts in the entry. Never provide more than 3
  themes: Array<{
    title: string; // A short title for this theme
    summary: string; // One sentence summarizing this theme
    keywords: string[]; // An array of keywords or key-phrases in this theme. Never provide more than 3
  }>
}
\`\`\``
const PROMPT = `Below is a TypeScript template which provides instructions for extracting information from a journal entry:
${TS_FORMAT}
Between >>> and <<< is the journal entry. Follow the template to extract the entry's information. Provide the output as valid JSON.
>>> [JOURNAL] <<<`
