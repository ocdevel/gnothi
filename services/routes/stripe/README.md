This file is for reference for webhooks.ts. May need to update which events we're listening for

#### From ChatGPT:
With your specific setup of a user-subscription model at x/month with a 7-day free trial, you'll want to listen for the following events to manage the `user.isPremiumActive` status:
1. `customer.subscription.created`: Triggered when a user starts a new subscription, including the start of a free trial. You could use this event to mark the user as having an active premium subscription (`user.isPremiumActive = true`), even though they're currently in their free trial.
2. `customer.subscription.updated`: This event fires when a subscription changes. You would want to listen for this event to handle the transition from the free trial to the paid period, as well as any changes the customer makes to their subscription.
3. `invoice.payment_succeeded`: This event is fired when an invoice payment is successful, such as when the free trial ends and the first actual payment is made. You could use this event to confirm that the user has begun their paid period after the free trial ends.
4. `customer.subscription.deleted`: This event is fired when a subscription is cancelled. You would want to listen for this event to handle the case where a user cancels their subscription, at which point you would set `user.isPremiumActive = false`.
Please note that these are basic events that most subscriptions would be interested in, but depending on your business logic and requirements, you might need to listen for more events. Additionally, remember that the exact logic of when to set `user.isPremiumActive = true` or `user.isPremiumActive = false` might vary depending on your specific application requirements.
Finally, it's always a good idea to test these events thoroughly to ensure they're working as expected in your application. You can simulate these events using the Stripe CLI to ensure your handlers are working correctly.

#### Me:
So if I only care about setting the one boolean, `isPremiumActive`, then is it correct that I truly only care about 1 & 4, and 2-3 are more informational?

#### GPT:
Correct. If all you care about is setting the `isPremiumActive` boolean in your system, you could technically get by with just listening to `customer.subscription.created` and `customer.subscription.deleted` events.
1. `customer.subscription.created`: This event would be triggered when a user starts a new subscription, including the start of a free trial. You could use this event to mark the user as having an active premium subscription (`user.isPremiumActive = true`).
2. `customer.subscription.deleted`: This event would be triggered when a user cancels their subscription. You would use this event to mark the user's premium subscription as inactive (`user.isPremiumActive = false`).
However, depending on the specifics of your implementation and what you want to achieve, you might find it useful to listen to other events as well. For example, `customer.subscription.updated` could be useful if you want to track changes to the subscription, such as when a user changes their plan, and `invoice.payment_succeeded` would let you know when a payment has successfully been made. But for the simple case of tracking whether a subscription is active or not, just listening to the `created` and `deleted` events should be sufficient.

#### Me:
And there's nothing really I need to watch for, for failed payments for the subscription? Will stripe be sort of squaring that away with other events and emails with them, and if all fails I get a `customer.subscription.deleted`? Or can they be in some payment-failed regime for multiple months and I wouldn't be notified, if I only used those 2?

GPT:
Stripe handles failed payments with an automatic retry system and customer communication. If a payment fails, Stripe will retry the charge according to a specific schedule, and also send email notifications to the customer asking them to update their payment information.

However, if the payment continues to fail after several attempts and the issue is not resolved (for example, if the customer does not update their expired card), the subscription will be cancelled. In that case, Stripe will send a `customer.subscription.deleted` event to your webhook endpoint. Listening to this event allows you to update the user's status in your system (i.e., `user.isPremiumActive = false`).

It's also possible to listen to other related events if you want to be informed about these situations in real time, for instance:

- `invoice.payment_failed`: Occurs whenever an invoice payment attempt fails.

This event can be useful if you want to take additional actions when a payment fails, such as sending a custom notification to the user or to your support team, or logging the event for analytics purposes.

But if your only concern is to keep the `isPremiumActive` status up to date, listening to `customer.subscription.deleted` should be enough, because Stripe will eventually cancel the subscription if the payment issue is not resolved.

```typescript
export function allEvents(event: any) {
  // Handle the event
  switch (event.type) {
