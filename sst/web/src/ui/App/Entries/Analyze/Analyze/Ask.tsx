...
export default function Ask() {
  const res = useStore(s => s.res.analyze_ask_response?.res)
  const job = useStore(s => s.res.analyze_ask_response?.row)
  const form = useStore(s => s.analyze.question)
  const reply = useStore(s => s.res.analyze_ask_final?.row)
