...
type Checked = 'custom:yesTerms_Disclaimer' | 'custom:yesPrivacyPolicy' | 'all'
export const AuthComponent: FC<AuthComponentProps> = ({tab}) => {
  ...
  const [checked, setChecked] = useState<Record<Checked, boolean>>({
    ...
    'all': false, // true if all checkboxes checked, and final NEXT button clicked
  })

  return <div className={checked.all ? '' : 'hide-signup-button'}>
    <Authenticator
      loginMechanisms={["email"]}
      initialState={tab}
      components={{
          ...
          FormFields() {
            ...
            if (step === 1) {
              return <Stack spacing={2}>
                ...
                <Button variant="contained" fullWidth disabled={!checked['custom:yesTerms_Disclaimer']} onClick={() => setStep(2)}>Next</Button>
              </Stack>
            }
            if (step === 2) {
              return <Stack spacing={2}>
